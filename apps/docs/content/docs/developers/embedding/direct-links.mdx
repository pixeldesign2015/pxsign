...
When someone uses a direct link, PxSign creates a new document from the template with that person as the signer.
...
curl -X POST "https://app.sign.pixeldesign.io/api/v2/template/direct/create" \
...
// URL: https://app.sign.pixeldesign.io/d/{token}
````
</Tab>
</Tabs>
...
```
https://app.sign.pixeldesign.io/d/{token}
```
...
```html
<iframe
  src="https://app.sign.pixeldesign.io/embed/direct/abc123xyz"
  width="100%"
  height="800"
  frameborder="0"
...
function DocumentSigner({ token }: { token: string }) {
  return (
    <div className="h-[800px] w-full">
      <iframe
        src={`https://app.sign.pixeldesign.io/embed/direct/${token}`}
...
function redirectToSigning(token: string) {
  window.location.href = `https://app.sign.pixeldesign.io/d/${token}`;
}
```
...
```
https://app.sign.pixeldesign.io/d/abc123xyz?externalId=order-12345
```
...
function getSigningUrl(token: string, orderId: string) {
  ...
  return `https://app.sign.pixeldesign.io/d/${token}?${params.toString()}`;
}
...
```
https://app.sign.pixeldesign.io/embed/direct/abc123xyz?externalId=order-12345
```
...
| Account   | Signer must be logged into a PxSign account |
...
<Tab value="curl">
```bash
curl -X POST "https://app.sign.pixeldesign.io/api/v2/template/direct/toggle" \
  -H "Authorization: api_xxxxxxxxxxxxxxxx" \
  -H "Content-Type: application/json" \
  -d '{
    "templateId": 123,
...
```bash
curl -X POST "https://app.sign.pixeldesign.io/api/v2/template/direct/delete" \
  -H "Authorization: api_xxxxxxxxxxxxxxxx" \
  -H "Content-Type: application/json" \
  -d '{
    "templateId": 123
...
Enterprise plans support white-label embedding with the "Powered by PxSign" badge removed. [Contact sales](mailto:support@sign.pixeldesign.io) for details.
...
function getEmbedUrl(token: string, orderId: string) {
  ...
  return `https://app.sign.pixeldesign.io/embed/direct/${token}?${params}`;
}
