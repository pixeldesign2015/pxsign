...
Register your endpoint in the PxSign dashboard:
...
class PxSignApiError extends Error {
  code?: string;
  statusCode: number;
  details?: Record<string, unknown>;
...
try {
  ...
  if (error instanceof PxSignApiError) {
...
- [API Reference](https://openapi.sign.pixeldesign.io/) - Full endpoint documentation
