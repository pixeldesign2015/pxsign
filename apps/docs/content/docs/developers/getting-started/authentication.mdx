...
- A PxSign account on any plan (Free, Individual, Team, or Enterprise)
...
```bash
curl https://app.sign.pixeldesign.io/api/v2/documents \
...
```typescript
const response = await fetch('https://app.sign.pixeldesign.io/api/v2/documents', {
  method: 'GET',
  headers: {
...
PxSign provides official SDKs that handle authentication for you:

```typescript
import { PxSign } from '@documenso/sdk-typescript';

const client = new PxSign({
...
<Callout type="info">
  ...
  OpenAPI reference](https://app.sign.pixeldesign.io/api/v1/openapi).
</Callout>
