...
description: Create and send a document for signing using the PxSign API, from uploading a PDF to adding recipients and distributing for signature.
---
...
- A PxSign account (cloud or self-hosted)
...
| ----------- | -------------------------------------- |
| Production  | `https://app.sign.pixeldesign.io/api/v2`     |
| Staging     | `https://stg-app.sign.pixeldesign.io/api/v2` |
...
<Tab value="curl">
```bash
curl -X POST "https://app.sign.pixeldesign.io/api/v2/envelope/envelope_abc123/distribute" \
  -H "Authorization: YOUR_API_TOKEN" \
...
const response = await fetch(
  `https://app.sign.pixeldesign.io/api/v2/envelope/${envelopeId}/distribute`,
  {
    method: 'POST',
    headers: {
      Authorization: 'YOUR_API_TOKEN',
...
BASE_URL="https://app.sign.pixeldesign.io/api/v2"
...
- [API Reference](https://openapi.sign.pixeldesign.io/) - Full endpoint documentation with request/response schemas
