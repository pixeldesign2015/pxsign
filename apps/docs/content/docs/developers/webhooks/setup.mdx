...
Webhooks are HTTP callbacks triggered by specific events in PxSign. When an event occurs (such as a document being signed), PxSign sends an HTTP POST request to your configured URL with details about the event.
...
Before configuring a webhook in PxSign, you need an endpoint that can receive HTTP POST requests. Here's a minimal example:
...
<Callout type="warn">
  Always respond with a `200 OK` status within 30 seconds. PxSign will retry failed deliveries.
</Callout>

## Configuring Webhooks in PxSign via the Dashboard
...
PxSign provides a built-in testing feature to verify your webhook endpoint works correctly.
...
<Step>
PxSign will attempt to deliver the same payload again
</Step>
</Steps>

## Retry Policy

When a webhook delivery fails (non-2xx response or timeout), PxSign automatically retries with exponential backoff:
...
<Accordions type="multiple">
  ...
  <Accordion title="Verify signatures">
    Use a webhook secret and verify the `X-PxSign-Secret` header (see [Verification](/docs/developers/webhooks/verification)).
  </Accordion>
