...
Verifying webhook requests ensures that incoming payloads originate from PxSign and have not been tampered with. Without verification, attackers could forge requests to your endpoint and trigger unintended actions.

## How PxSign Signs Webhooks

When you configure a webhook with a secret, PxSign includes that secret in every webhook request via the `X-PxSign-Secret` header. Your server should compare this header value against your stored secret to authenticate the request.

```bash
POST /your-webhook-endpoint HTTP/1.1
Host: your-server.com
Content-Type: application/json
X-PxSign-Secret: your_webhook_secret_here
...
| Header               | Description                                             |
| -------------------- | ------------------------------------------------------- |
| `X-PxSign-Secret` | The secret key you configured when creating the webhook |
...
<Steps>
  <Step>
    Extract the `X-PxSign-Secret` header from the incoming request
  </Step>
  <Step>
...
def handle_webhook():
    received_secret = request.headers.get('X-PxSign-Secret', '')
...
| --------------- | --------------------------------------- | ------------------------------------------------------------------ |
| Secret mismatch | Webhook secret changed or misconfigured | Verify the secret in your environment matches the one in PxSign |
| Empty header    | Webhook created without a secret        | Add a secret to the webhook configuration in PxSign             |
...
3. Update the webhook configuration in PxSign
