...
## How PxSign Supports Compliance

PxSign provides features that support compliance with e-signature laws across jurisdictions:
...
PxSign maintains an audit trail for each document, recording:
...
## What PxSign Does NOT Provide

PxSign supports compliance with Simple Electronic Signature (SES) requirements. The following are not currently provided:
