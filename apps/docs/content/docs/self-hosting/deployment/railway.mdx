---
title: Railway
description: Deploy PxSign on Railway with automatic builds from Git, managed PostgreSQL, and built-in networking. Use the one-click template or manual setup.
---
...
Click the URL in the Railway dashboard to access your PxSign instance.
...
If you prefer more control over the setup, deploy PxSign manually:
...
- Fork or select the [PxSign repository](https://github.com/documenso/documenso)
...
Click on the PxSign service and go to the "Variables" tab. Add the following variables:
...
NEXT_PRIVATE_SMTP_FROM_NAME=PxSign
...
PxSign runs database migrations automatically on startup.
...
Railway provides several options for scaling your PxSign deployment:
...
<Steps>
<Step>
### Open your PxSign service

In the Railway dashboard, select the PxSign service.
...
In the Railway dashboard, select your PxSign service and open "Settings".
...
<Callout type="warn">
  PxSign stores sessions in the database, so horizontal scaling works without additional
...
Railway uses usage-based pricing. Estimated monthly costs for PxSign:
...
PxSign requires a signing certificate to sign documents. On Railway, you can:

### Option 1: Use Generated Certificate (Default)

PxSign generates a self-signed certificate automatically if none is provided. This works for testing but shows generic certificate information in signed documents.
...
In your PxSign service variables, set:
