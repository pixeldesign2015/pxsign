---
title: Docker
description: Run PxSign as a standalone Docker container with an external database. Ideal for container platforms like AWS ECS, Google Cloud Run, or Azure Container Instances.
---
...
These variables must be set for PxSign to start:
...
| `NEXT_PUBLIC_WEBAPP_URL`                | Public URL where PxSign is accessible (e.g., `https://sign.example.com`) | -                             |
...
PxSign requires two database connection strings:
...
docker run -d \
  ...
  -e NEXT_PRIVATE_SMTP_FROM_NAME="PxSign" \
...
PxSign provides health check endpoints for monitoring:
...
To update PxSign to a new version:
...
The PxSign container is stateless. All persistent data is stored in the PostgreSQL database.
