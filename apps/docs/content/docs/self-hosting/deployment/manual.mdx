...
description: Deploy PxSign on a Linux server without Docker using Node.js and systemd.
---
...
<Callout type="info">
  If you use a reverse proxy in front of PxSign, set `NEXT_PUBLIC_WEBAPP_URL` to the public URL
...
Create a service file to run PxSign as a background service on Linux:
...
A minimal Nginx configuration for proxying to PxSign:
