...
description: Complete reference for all environment variables used to configure a self-hosted PxSign instance.
---

## Required Variables

These variables must be set for PxSign to function:
...
| `NEXT_PUBLIC_WEBAPP_URL`                | Public URL of your PxSign instance (e.g., `https://sign.example.com`)                   |
...
PxSign requires PostgreSQL 14 or higher.
...
PxSign supports multiple email transports for sending notifications.
...
PxSign can store documents in the database or S3-compatible storage.
...
PxSign requires a certificate to digitally sign documents.
...
PxSign can use Google Vertex AI for recipient and field detection.
...
PxSign supports multiple background job providers for processing emails, documents, webhooks, and scheduled tasks.
...
These variables require an active [Enterprise Edition](/docs/policies/enterprise-edition) license. Obtain a license key from [license.sign.pixeldesign.io](https://license.sign.pixeldesign.io) and set it below to unlock enterprise features such as SSO, embed authoring, and 21 CFR Part 11 compliance.
