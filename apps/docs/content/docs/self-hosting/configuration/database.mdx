...
description: Configure PostgreSQL connection strings, pooling, SSL, migrations, and performance tuning for your PxSign deployment.
---
...
PxSign requires **PostgreSQL 14 or later**. No other databases are supported.
...
PxSign uses two database connection variables:
...
PxSign automatically detects common database environment variable formats used by hosting providers:
...
If your hosting provider sets these variables, PxSign will use them automatically.
...
PxSign uses Prisma, which maintains its own connection pool. Configure pool size with connection string parameters:
...
Database migrations update your schema when upgrading PxSign.

### Automatic Migrations

When running PxSign via Docker, migrations run automatically on container startup. No manual intervention is required.
...
Key parameters for PxSign workloads:
...
PxSign includes necessary indexes by default. Additional indexes may help for:
