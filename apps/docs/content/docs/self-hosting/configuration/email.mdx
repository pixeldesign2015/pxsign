...
PxSign supports four email transport methods:
...
SMTP is the most common email transport. PxSign supports two SMTP authentication methods.
...
Generate an API key in the Resend dashboard for PxSign to use.
...
PxSign uses React Email templates for all outgoing emails. The templates are located in `packages/email/templates/` and include:
...
- Custom logo replaces the PxSign logo
...
Configure PxSign to use the test server:
...
NEXT_PRIVATE_SMTP_FROM_NAME="PxSign Test"
```
...
For MailChannels, configure DKIM directly in PxSign using the environment variables described above.
