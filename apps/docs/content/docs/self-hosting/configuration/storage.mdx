...
S3 storage is recommended for production deployments. PxSign supports AWS S3 and any S3-compatible storage service.
...
Using Docker Compose with PxSign:
...
PxSign works with any S3-compatible storage service. Configure the endpoint and enable path-style URLs if required.
...
- An S3 bucket configured for PxSign
...
PxSign does not provide automatic migration between storage backends. Each document's storage location is recorded in the database.
...
During migration, PxSign can read from both backends. The `DocumentData.type` field indicates where each document is stored.
