...
description: Configure Google Vertex AI so PxSign can detect recipients and fields automatically.
---
...
This guide covers how to enable the AI recipient and field detection features when you self-host PxSign.
...
- PxSign version that includes the AI detection feature and the corresponding database migration.
...
<Callout type="info">
  ...
  If you omit the location, PxSign uses `global`. Not all models are available in every region;
  if a model is unavailable, switch to a supported region.
</Callout>

## Deploy with the Published Container

- Use the official PxSign image (DockerHub or GHCR) and supply the Vertex env vars above.
