---
title: Requirements
description: System requirements and prerequisites for self-hosting PxSign.
---
...
PxSign requires the following external services:
...
PxSign uses PostgreSQL for all data storage including documents, users, and audit logs. You cannot use MySQL, SQLite, or other databases.

PxSign requires two connection strings:
...
PxSign sends transactional emails for:
...
For production deployments, place PxSign behind a reverse proxy such as:
...
The reverse proxy handles SSL/TLS termination and forwards requests to PxSign on port 3000.
...
By default, PxSign stores documents in the PostgreSQL database. For production deployments with significant document volume, use S3-compatible storage:
...
PxSign processes background jobs (email delivery, document processing) using a PostgreSQL-based queue by default. No additional services are required: the job queue is built into the application and uses your existing database.

For production deployments that need higher throughput or more reliable job processing, PxSign supports [BullMQ](https://docs.bullmq.io/) as an alternative provider. BullMQ requires a **Redis** instance (v6.2+). Set `NEXT_PRIVATE_JOBS_PROVIDER=bullmq` and configure `NEXT_PRIVATE_REDIS_URL`.
...
- [ ] Domain name for your PxSign instance
...
- [Quick Start](/docs/self-hosting/getting-started/quick-start) - Deploy PxSign with Docker in 5 minutes
