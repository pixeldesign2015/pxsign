...
test('[TEMPLATE]: should create a document from a template', async ({ page }) => {
  ...
  await page.getByLabel('Redirect URL').fill('https://sign.pixeldesign.io');
  ...
  await page.getByPlaceholder('Email').nth(1).fill('recipient2@sign.pixeldesign.io');
  ...
  expect(document.documentMeta?.redirectUrl).toEqual('https://sign.pixeldesign.io');
...
test('[TEMPLATE]: should create a document from a template with custom document', async ({
  ...
  // Add a signer
  await page.getByPlaceholder('Email').fill('recipient@sign.pixeldesign.io');
