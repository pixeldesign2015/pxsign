...
const addSignerAndSave = async (page: Page) => {
  await page.getByPlaceholder('Email').fill('recipient1@sign.pixeldesign.io');
...
test.describe('AutoSave Signers Step', () => {
  test('should autosave the signers addition', async ({ page }) => {
    ...
    await expect(async () => {
      ...
      expect(retrievedRecipients[0].email).toBe('recipient1@sign.pixeldesign.io');
  ...
  test('should autosave the signer update', async ({ page }) => {
    ...
    await page.getByPlaceholder('Name').fill('PxSign Manager');
    await page.getByPlaceholder('Email').fill('manager@sign.pixeldesign.io');
    ...
    await expect(async () => {
      ...
      expect(retrievedRecipients.length).toBe(1);
      expect(retrievedRecipients[0].email).toBe('manager@sign.pixeldesign.io');
      expect(retrievedRecipients[0].name).toBe('PxSign Manager');
  ...
  test('should autosave the signing order change', async ({ page }) => {
    ...
    await page.getByTestId('signer-email-input').nth(1).fill('recipient2@sign.pixeldesign.io');
    await page.getByLabel('Name').nth(1).fill('Recipient 2');

    await page.getByRole('button', { name: 'Add Signer' }).click();

    await page.getByTestId('signer-email-input').nth(2).fill('recipient3@sign.pixeldesign.io');
    ...
    await expect(async () => {
      ...
      const firstRecipient = retrievedRecipients.find(
        (r) => r.email === 'recipient1@sign.pixeldesign.io',
      );
      const secondRecipient = retrievedRecipients.find(
        (r) => r.email === 'recipient2@sign.pixeldesign.io',
      );
      const thirdRecipient = retrievedRecipients.find(
        (r) => r.email === 'recipient3@sign.pixeldesign.io',
      );
